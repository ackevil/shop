...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function(){
				//衣物类别
				displayType();
				displayCloth('${requestScope.clothList}');
				/* displayNav('${requestScope.pageNum}'); */
				dispalyMyNav(parseInt('${requestScope.pageNum}'),1,1);
			});
			function displayType(){
				var clothTypes = '${requestScope.ctInfo}';
				var clothTypeObjs = $.parseJSON(clothTypes);
				var node = "<option value='0'>衣物类别</option>";
				$("#type1").append(node);
				$.each(clothTypeObjs,function(n,v){
					node = "<option value='"
			  			+ v.clothTypeId
			  			+ "'>"
			  			+ v.clothTypeName
			...
			function displayCloth(clothInfo){
				//在这里首先remove所有的li标签
				$("#clothList li").remove();
				var clothObjs = $.parseJSON(clothInfo);
				var node = "";
				if(!clothInfo){
					node="<li style='height: 175px; text-align:center; font-size:13px'>没有符合您搜索条件的商品</li>";
  					$("#clothList").append(node);
				}else{
					$.each(clothObjs,function(n,v){
						node="<li class='mui-table-view-cell mui-media mui-col-xs-6'>"
			            +		"<a href='#' onClick='hit()' >"
				        +        	"<img class='mui-media-object mui-media-img' style='height:272px;width:360px;' src='<%=basePath%>" + v.clothMainpicPath + "'>"
				        ...
				        +        		"<div class='lingqu'>";
				        if(v.clothState != 0){
				        	node += "<button class='getButton' style='background-color:#cfcfcf'>已领取</button>";
				        }else{
				        	node += "<button class='getButton'>领取</button>";
				        }
				        node +=  		"</div>"
			            +    		"</div>"
			            +		"</a>"
			...
			function refreshPage(){
				refreshList();
				dispalyMyNav(parseInt($("#pageNum").val()),1,1);
			}
			function refreshList(pageNo){
				...
				$.ajax({
			        ...
			        type:"POST",
			        url:"<%=basePath%>pc/yjya/refreshList_do",
			        async:false,
			        error:function(data){
			            alert("出錯了！");
			        },
			        success:function(data){
			            ...
			            if (data.clothInfo!="") {
			            	...
			            	/* displayNav(data.pageNum); */
			            	displayCloth(data.clothInfo);
			            }
			            else {
			                 alert("出錯了！");
			            }
	...
	<body>
		<div class="box">
			<div class="top">
				...
				<div class="top_2">
					<ul>
						...
						<a href="<%=basePath%>pc/yjya" style="color: #00BCE4;"><li>依旧有爱</li></a>
						<a href="<%=basePath%>pc/dsze"><li>滴水之恩</li></a>
						<a href="<%=basePath%>pc/zdjz"><li>重大救助</li></a>
			...
			<div class="middle" style="height: 1100px;">
				<div class="middle_box">
					<div class="middle_box-top">											
						<select id="type1" onchange="refreshPage()" style="width: 100px;margin-top: 30px;border: 1px solid #D0D0D0;float: left;">
						</select>
						<select id="type2" onchange="refreshPage()" style="width: 100px;margin-top: 30px;border: 1px solid #D0D0D0;float: left;margin-left: 30px;">
							<option value="0">性别</option>
							<option value="1">男</option>
							<option value="2">女</option>
						</select>
						<select id="type3" onchange="refreshPage()" style="width: 100px;margin-top: 30px;border: 1px solid #D0D0D0;float: left;margin-left: 30px;">
						...
						<button id='alertBtn' class="juanzhuButton" onClick="hit()">我要捐助</button>
					</div>
					<div class="showarea">
						<ul id="clothList" class="mui-table-view mui-grid-view" style="height: 850px;">
			      		</ul>
					</div>
