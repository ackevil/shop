	...
	<head>
		...
		<script type="text/javascript">
			$(function(){
				//商品类别
				displayType();
				displayProduct('${requestScope.productList}');
			...
			function displayType(){
				...
				var node = "<option value='0'>商品类别</option>";
				$("#type1").append(node);
			...
			function displayProduct(productInfo){
				...
				var productObjs = $.parseJSON(productInfo);
				var node = "";
				if(!productObjs){
					node="<li style='height: 175px; text-align:center; font-size:13px'>没有符合您搜索条件的商品</li>";
  					$("#productList").append(node);
				}else{
					$.each(productObjs,function(n,v){
						node="<li class='mui-table-view-cell mui-media mui-col-xs-6'>"
			            +		"<a href='#' onClick='hit()' >"
				        +        	"<img class='mui-media-object mui-media-img' style='height:272px;width:360px;' src='<%=basePath%>" + v[3] + "'>"
				        +        	"<div class='mui-media-body'>"
				        +        		"<div class='title'>"
				        +        		v[1]
				        +        		"</div>"
				        +        		"<div class='lingqu'>";
				        node += "<button class='getButton'>购买</button>";
				        node +=  		"</div>"
			            +    		"</div>"
			            +		"</a>"
			        	+	"</li>";
			...
			function refreshList(pageNo){
				...
				$.ajax({
			        ...
			        type:"POST",
			        url:"<%=basePath%>pc/aigou/refreshList_do",
			        async:false,
			        error:function(data){
			            alert("出錯了！");
			        },
			        success:function(data){
			            ...
			            if (data.productInfo!="") {
			            	...
			            	displayProduct(data.productInfo);
			            }
			            else {
			                 alert("出錯了！");
			            }
		...
		<div class="box">
			<div class="top">
				...
				<div class="top_2">
					<ul>
						...
						<a href="<%=basePath%>pc/aigou" style="color: #00BCE4;"><li>爱心购</li></a>
					</ul>
				</div>
				<div class="top_3">
			...
			<div class="middle" style="height: 1100px;">
				<div class="middle_box">
					...
					<div class="showarea">
						<ul id="productList" class="mui-table-view mui-grid-view" style="height: 850px;">
			      		</ul>
					</div>
